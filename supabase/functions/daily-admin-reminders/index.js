...
const sendAdminSms = async (to, message) => {
...
serve(async (req) => {
  ...
  try {
    ...
    const dueForSixWeekReminder = [...groupedByDog.values()].filter((booking) => booking.confirmed_date === sixWeeksDate);

    if (dueForSixWeekReminder.length > 0) {
      const lines = dueForSixWeekReminder.map((booking) => {
    ...
    const tomorrowBookings = (confirmedBookings || []).filter((booking) => booking.confirmed_date === tomorrowDate);

    if (tomorrowBookings.length > 0) {
      const summary = tomorrowBookings.map((booking) => `${booking.confirmed_time || "TBC"} ${booking.dogname} (${booking.serviceid})`).join(", ");
      await sendAdminEmail(reminderEmail, `Tomorrow's grooms (${formatDate(tomorrowDate)})`, `Tomorrow's grooms (${formatDate(tomorrowDate)}): ${tomorrowBookings.length}. ${summary}`);
    }
