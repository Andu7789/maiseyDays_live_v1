...
export const RoundTimePicker: React.FC<RoundTimePickerProps> = ({ value, onChange, className }) => {
  ...
  // Clock order: index 0 sits at the top (12 o'clock), then 1..11 run clockwise.
  const outerHours = Array.from({ length: 12 }, (_, i) => {
    const value = i === 0 ? 12 : i;
    return { label: String(value), value };
  });
  // Inner ring mirrors the same positions: 24 (midnight) at the top, 13..23 clockwise.
  const innerHours = Array.from({ length: 12 }, (_, i) => {
    if (i === 0) return { label: "24", value: 0 };
    const value = 12 + i;
    return { label: String(value), value };
  });
